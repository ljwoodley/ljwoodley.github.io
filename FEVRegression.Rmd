...
# Load libraries (load plyr before dplyr so that dplyr's awesomeness can prevail)
pacman::p_load(
car,
MASS,
zoo,
...
```{r echo=FALSE, fig.align="center",fig.cap= "Residual Analysis (model 1)",fig.width=7, fig.height=5}
...
```{r echo=FALSE, fig.width=7, fig.height=5,fig.align="center", fig.cap="Residual Analysis (model 2 )"}
...
```{r,fig.height=3.5, fig.align="center", fig.cap="Influential Observations"}
par(mfrow=c(1,2))
...
kable(b,"html",caption="Bonferroni Outlier Test",align = 'c') %>% 
...
colnames(df) <- c("Observation #","DFFITS", "DFBETA\nIntercept","DFBETA\nAge",
                 "DFBETA\nHeight","DFBETA\nSex","DFBETA\nSmoke","Cook's\nDistance","Leverage")
kable(df,"html",caption="Influential Measures") %>% 
...
summary(model2) 
```
```{r, fig.height=3,fig.align="center",fig.cap="Figure 4: VIF" }
pic <- plot_model(model2,type="diag")
pic[[1]]
```
...
Kavitha, A., Sujatha, C. M., & Ramakrishnan, S. (2010). Prediction of Spirometric Force Expiratory Volume (FEV1) Data Using Support    Vector Regression. Measurement Science Review, 10(2). Retrieved from http://www.measurement.sk/2010/S1/Kavitha.pdf.

Kutner, H., Nachtsheim, C., Neter, J., & William, L. (2005). Applied Linear Statistical Models (5th ed.). Boston: McGraw-Hill Irwin.
